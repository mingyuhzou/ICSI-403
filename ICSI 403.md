...
不允许改小节点的值，如果节点的值修改后能大于器父节点，那么就将节点与父节点交换，然乎向上检查
...
霍夫曼树可用于构建**最短前缀编码**，以字符出现的频率为权重构建霍夫曼树(权重只在建树中有用)
...
4. 当前节点**父节点以及叔父节点均为红色**(优先级更高)，则将**二者染黑**，**祖父染红**，递归维护**祖父节点**，如果祖父节点是根节点，那么需要染黑，即出现连续的黑色的情况<img src="./assets/image-20250408173416581.png" alt="image-20250408173416581" style="zoom:67%;" />
